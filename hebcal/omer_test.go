...
func TestSefira(t *testing.T) {
	assert := assert.New(t)
	omer := hebcal.NewOmerEvent(hdate.New(5770, hdate.Sivan, 2), 46)
...
func TestTodayIsHe(t *testing.T) {
	...
	startAbs := start.Abs()
	for i := 1; i <= 49; i++ {
		ev := hebcal.NewOmerEvent(hdate.FromRD(startAbs+i-1), i)
